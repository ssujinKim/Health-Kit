...
# 각 영양소 값을 추출하고, None 값이면 0을 기본값으로 사용
...
def nat_rec(person, df, df2, sor, case, med, mtf):
    # 개인의 영양 상태 확인
    case.extend(check_min(person))
    df = df[df['분류'] == "밥류"]
    person_nutrients = person.extra_nut()
    kcal, carbo, sugar, fats, trans_fats, sat_fats, cholesterol, protein, calcium, sodium = person_nutrients
    ...
    for num in case:
        ...
        # 여기서부터는 영양성분 초과한 case
            ...
            merged_df = sort_and_merge(merged_df, sor, '식이섬유(g)', percentage=0.3)
...
#print(kcal, carbs, sugars, fat, trans_fat, saturated_fat, cholesterol, protein, calcium, sodium)
#print(preferences_list)

result = nat_rec(person, df, dis, sor, preferences_list, med, mtf)

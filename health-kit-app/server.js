...
app.post('/updateUserInfo', async (req, res) => {
  ...
  try {
    await db.query(
      'UPDATE USERS SET height = ?, weight = ?, age = ?, gender = ?, disease1 = ?, disease2 = ?, disease3 = ?, medicine1 = ?, medicine2 = ?, medicine3 = ? WHERE email = ?',
      [
        updatedInfo.height,
        updatedInfo.weight,
        updatedInfo.age,
        updatedInfo.gender,
        updatedInfo.disease1,
        updatedInfo.disease2,
        updatedInfo.disease3,
        updatedInfo.medicine1,
        updatedInfo.medicine2,
        updatedInfo.medicine3,
        updatedInfo.email,
      ]
    );
    ...
    if (foodInfo.length > 0) {
      ...
      await db.query(
        ...
        [
          ...
          foodInfo[0].sugars,
          foodInfo[0].fat,
          foodInfo[0].trans_fat,
          foodInfo[0].saturated_fat,
          foodInfo[0].cholesterol,
          foodInfo[0].protein,
          foodInfo[0].calcium,
          foodInfo[0].sodium,
        ]
...
app.post('/deleteMeal', async (req, res) => {
  ...
  try {
    await db.query(
      'DELETE FROM users_diet WHERE user_email = ? AND date = ? AND food_name = ? AND meal_type = ?',
...
app.get('/run-python-ocr', async (req, res) => {
  ...
  const {email, date, productName, amount, calories, calorieType} = req.query;
  ...
  try {
    ...
    const [dietResults] = await db.query(
      `SELECT SUM(kcal) AS totalCalories, SUM(carbs) AS totalCarbs, 
    ...
    const pythonProcess = spawn(
      'C:/Health-Kit/Health-Kit/health-kit-app/myvenv/Scripts/python.exe',
      ['./model/ocr_recommendations.py', userInfo, dietInfo, productName, amount, calories, calorieType],
      {
...
app.get('/run-python-ocr2', async (req, res) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  const {email, date, productName, amount, calories, calorieType} = req.query;
  ...
  try {
    ...
    const pythonProcess = spawn(
      'C:/Health-Kit/Health-Kit/health-kit-app/myvenv/Scripts/python.exe',
      ['./model/ocr_similar_food.py', userInfo, dietInfo, productName, amount, calories, calorieType],
      {
        env: {...process.env, PYTHONIOENCODING: 'utf-8'}, // 한글 안 깨지게
      }
    );
